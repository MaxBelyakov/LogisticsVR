...
namespace Trucks
{
    ...
    public class TruckAIControl : MonoBehaviour
    {
        ...
        private TruckController m_CarController;    // Reference to actual car controller we are controlling
        ...
        public bool startMoving;                    // Flag when car start moving to give it time to increase speed before control get stuck flag
        private bool startTimer;                    // Time controller to increase car speed
        ...
        private void Awake()
        {
            ...
            m_CarController = GetComponent<TruckController>();
        ...
        {
            // Fix: when the car start moving the speed is low and car can choose get stuck behavior
            if (startMoving)
            ...
            else
            {
                ...
                // Change acceleration vector and target angle when moving back
                if (moveBack)
                {
                    accel = accel * -1;

                    // Fix: move back truck angle correction to get fluent turn
                    targetAngle = Mathf.Atan2(localTarget.x, localTarget.z + 100f)*Mathf.Rad2Deg;
                }
        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            {
                var otherAI = col.rigidbody.GetComponent<TruckAIControl>();
                if (otherAI != null)
                {
